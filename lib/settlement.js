...
export function calculateSmartSettlements(members = [], expenses = []) {
  ...
  (expenses || []).forEach((expense) => {
    ...
    // 1. Credit the payer(s)
    if (expense.paidByShares && typeof expense.paidByShares === "object" && Object.keys(expense.paidByShares).length > 0) {
      Object.entries(expense.paidByShares).forEach(([person, paidVal]) => {
        const pAmount = Number(paidVal) || 0;
        if (pAmount > 0) {
          balances[person] = (balances[person] || 0) + pAmount;
        }
      });
    } else if (paidBy && balances[paidBy] !== undefined) {
      balances[paidBy] = (balances[paidBy] || 0) + amount;
    } else if (members.length > 0) {
      // Fallback if paidBy name is unrecognized
      balances[members[0]] = (balances[members[0]] || 0) + amount;
    }

    if (expense.memberShares && typeof expense.memberShares === "object" && Object.keys(expense.memberShares).length > 0) {
      // The splitters are debited according to their itemized shares
      Object.entries(expense.memberShares).forEach(([person, share]) => {
        const shareAmount = Number(share) || 0;
        if (shareAmount > 0) {
      ...
      // The splitters are debited equally
