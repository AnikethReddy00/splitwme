...
export function createGroup({ name, category, creatorName, creatorUpi, creatorAvatar, members = [], memberDetails = {} }) {
  const groups = getAllGroups();
  ...
  const newGroup = {
    ...
    memberDetails: {
      [creatorName]: {
        ...
        avatar: creatorAvatar || `https://api.dicebear.com/7.x/bottts/svg?seed=${encodeURIComponent(creatorName)}`
      },
      ...memberDetails
    },
    expenses: []
...
export function removeMemberFromGroup(groupId, memberName) {
  ...
  group.expenses?.forEach((expense) => {
    ...
    if (expense.paidByShares && expense.paidByShares[memberName]) {
      delete expense.paidByShares[memberName];
    }
    if (expense.memberShares && expense.memberShares[memberName]) {
      delete expense.memberShares[memberName];
    }
  });
...
export function addExpenseToGroup(groupId, expense) {
  ...
  const newExpense = {
    ...
    paidByShares: expense.paidByShares || null,
    ...
    memberShares: expense.memberShares || null,
    items: expense.items || null,
    tax: Number(expense.tax) || 0,
    serviceCharge: Number(expense.serviceCharge) || 0,
    extraCharges: Number(expense.extraCharges) || 0
  };
...
export function updateExpenseInGroup(groupId, expenseId, updatedData) {
  ...
  if (updatedData.paidByShares !== undefined) expense.paidByShares = updatedData.paidByShares;
  ...
  if (updatedData.memberShares !== undefined) expense.memberShares = updatedData.memberShares;
  if (updatedData.items !== undefined) expense.items = updatedData.items;
  if (updatedData.tax !== undefined) expense.tax = Number(updatedData.tax) || 0;
  if (updatedData.serviceCharge !== undefined) expense.serviceCharge = Number(updatedData.serviceCharge) || 0;
  if (updatedData.extraCharges !== undefined) expense.extraCharges = Number(updatedData.extraCharges) || 0;
